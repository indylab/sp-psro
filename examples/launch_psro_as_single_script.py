...
if __name__ == '__main__':
    ...
    manager = launch_manager(scenario=scenario, psro_port=psro_port, eval_port=eval_port, block=False,
                             include_evals=False, restore_payoff_table_path=commandline_args.restore)

    br_method = commandline_args.br_method
    use_prev_brs = commandline_args.use_prev_brs
    instant_first_iter = commandline_args.instant_first_iter
    if br_method == "standard":
        ...
        for player in range(2):
            command = ["python", br_script_path, "--scenario", scenario_name, "--player", str(player)]
            if use_prev_brs:
                command.append("--use_prev_brs")
            if instant_first_iter:
                command.append("--instant_first_iter")
