...
class ReservoirReplayBuffer:
    ...
    def __init__(self, size: int):
        ...
        self._add_calls = 0
        print(f"Creating Reservoir buffer with max size {self._maxsize}")
    ...
    def add(self, item: SampleBatchType):
        ...
        item = copy.deepcopy(item)  # defensive copy to allow original item to be garbage collected
        ...
        if self._next_idx is not None:
            ...
            assert self._num_timesteps_added >= self._maxsize
            self._sample_reservior_buffer_next_index()
        self._add_calls += 1

    def _encode_sample(self, idxes: List[int]) -> SampleBatchType:
        out = SampleBatch.concat_samples([copy.deepcopy(self._storage[i]) for i in idxes])  # defensive copy
