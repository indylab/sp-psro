...
class EvalResult(object):
    ...
    def __init__(self,
                 policy_specs_for_each_player: List[StrategySpec],
                 payoff_for_each_player: List[float],
                 games_played: int):
...
class EvalResultPostProcessing(ABC):

    @abstractmethod
    def __call__(self, eval_result: EvalResult) -> EvalResult:
        raise NotImplementedError


class EvalDispatcher(object):

    def __init__(self, games_per_eval: int,
        ...
        self._eval_result_post_processing = eval_result_post_processing
    ...
    def submit_eval_job_result(self, policy_specs_for_each_player_tuple, payoffs_for_each_player: List[float],
        ...
        if self._eval_result_post_processing is not None:
            eval_result = self._eval_result_post_processing(eval_result=eval_result)
