...
from grl.algos.p2sro.eval_dispatcher import EvalDispatcherWithServer, EvalResult, EvalResultPostProcessing
...
class P2SROManager(object):
    ...
    def __init__(self,
                 ...
                 restore_payoff_table_json_file_path: str = None):
        ...
        self._player_stats = [_P2SROPlayerStats(player=i) for i in range(n_players)]

        if restore_payoff_table_json_file_path is not None:
            self.restored_from_path = restore_payoff_table_json_file_path
            self._payoff_table = PayoffTable.from_json_file(json_file_path=restore_payoff_table_json_file_path)
            assert n_players == self._payoff_table.n_players()
            for p in range(n_players):
                # We want all previous entries in the payoff table to have complete info. Can't have payoff table cells that are missing results
                player_games_played_matrix = self._payoff_table.get_games_played_matrix_for_player(player=p)
                assert 0 not in player_games_played_matrix or np.size(player_games_played_matrix) == 1 or np.all(
                    np.diagonal(player_games_played_matrix) == 0), player_games_played_matrix
            # All players have the same number of strats. Things may work if this isn't true, but it isn't tested.
            assert len(set(self._payoff_table.shape())) == 1
            num_fixed_policies_per_player = self._payoff_table.shape()

            for p in range(n_players):
                num_fixed_policies_for_player = None
                try:
                    num_fixed_policies_for_player = num_fixed_policies_per_player[p]
                except IndexError:
                    pass
                if num_fixed_policies_for_player:
                    self._player_stats[p] = _P2SROPlayerStats(player=p)
                    self._player_stats[p].fixed_policy_indexes = list(range(num_fixed_policies_for_player))
                    self._player_stats[p]._next_active_policy = num_fixed_policies_for_player

        else:
            self.restored_from_path = None
    ...
    # # We probably can delete this
    # @classmethod
    # def from_payoff_table_checkpoint(cls,
    #                                  payoff_table_json_file_path: str,
    #                                  n_players,
    #                                  is_two_player_symmetric_zero_sum: bool,
    #                                  do_external_payoff_evals_for_new_fixed_policies: bool,
    #                                  games_per_external_payoff_eval: int,
    ...
    #                                  manager_metadata: dict = None):
    #
    #     payoff_table = PayoffTable.from_json_file(json_file_path=payoff_table_json_file_path)
    #     assert n_players == payoff_table.n_players()
    #
    #     for p in range(n_players):
    #         # We want all previous entries in the payoff table to have complete info. Can't have payoff table cells that are missing results
    #         player_games_played_matrix = payoff_table.get_games_played_matrix_for_player(player=p)
    #         assert 0 not in player_games_played_matrix or np.size(player_games_played_matrix) == 1 or np.all(
    #             np.diagonal(player_games_played_matrix) == 0), player_games_played_matrix
    #
    #     manager = P2SROManager(n_players=n_players,
    ...
    #                            log_dir=log_dir)
    #
    #     manager._payoff_table = payoff_table
    #     # All players have the same number of strats. Things may work if this isn't true, but it isn't tested.
    #     assert len(set(payoff_table.shape())) == 1
    #     num_fixed_policies_per_player = payoff_table.shape()
    #     for p in range(n_players):
    #         manager._player_stats[p] = _P2SROPlayerStats(player=p)
    #         manager._player_stats[p].fixed_policy_indexes = list(range(num_fixed_policies_per_player[p]))
    #         manager._player_stats[p]._next_active_policy = num_fixed_policies_per_player[p]
    #     if manager._is_two_player_symmetric_zero_sum:
    #         # both array entries point to the same stats
    #         manager._player_stats[1] = manager._player_stats[0]
    #     return manager
    ...
    def claim_new_active_policy_for_player(self, player, new_policy_metadata_dict, set_as_fixed_immediately=False) -> StrategySpec:
        with self._modification_lock:
            ...
            if set_as_fixed_immediately:
                new_policy_spec = self.set_active_policy_as_fixed(
                    player=player, policy_num=new_active_policy_num, final_metadata_dict=new_policy_metadata_dict)
    ...
    def _on_finished_eval_result(self, eval_result: EvalResult):
        with self._modification_lock:
            ...
            # filter out extra agents if there are any
            eval_result.policy_specs_for_each_player = eval_result.policy_specs_for_each_player[:self._n_players]
            eval_result.payoff_for_each_player = eval_result.payoff_for_each_player[:self._n_players]
            ...
            # iterate by order of increasing policy nums
            for key, matchups_for_fixed_policy in sorted(self._pending_spec_matchups_for_new_fixed_policies.items(), key=lambda x: x[0][1]):
                ...
                if (len(matchups_for_fixed_policy) == 0 and
                        np.array_equal(self._player_stats[player].fixed_policy_indexes,
                                       list(range(0, new_fixed_policy_num)))):
                    # No more pending matchups and player policies below this one are fixed
                    ...
                    print(f"player: {player} active policies: {self._player_stats[player].active_policy_indexes}, fixed policies: {self._player_stats[player].fixed_policy_indexes}")
                    ...
                    def _logger_callback(player=player,
                                         new_fixed_policy_num=new_fixed_policy_num,
                                         fixed_policy_spec=fixed_policy_spec):
                        self._manager_logger.on_active_policy_moved_to_fixed(player=player,
                                                                             policy_num=new_fixed_policy_num,
                                                                             fixed_policy_spec=fixed_policy_spec)
                    logger_callbacks.append(_logger_callback)
            ...
            # Do logger callbacks after payoff results are entered in the payoff table in case those results are used.
