...
class GoofspielMultiAgentEnv(ValidActionsMultiAgentEnv):
    ...
    def __init__(self, env_config=None):
        ...
        assert self.game_version == "goofspiel"
        ...
        self._armac_variant = env_config["armac_variant"]

        self.openspiel_env, self.open_spiel_env_config = self._get_openspiel_env()
        ...
        self._illegal_actions_default_to_min_allowed_action = bool(env_config.get("illegal_actions_default_to_min_allowed_action", False))
    ...
    def _get_openspiel_env(self):
        if self._armac_variant:
            return Environment(self.game_version, imp_info=True, num_cards=5, points_order="descending"), {"imp_info": True, "num_cards": 5, "points_order": "descending"}

        return Environment(game=self.game_version), {}
    ...
    def step(self, action_dict):
        ...
        for player in [0, 1]:
            ...
            if player_action not in legal_actions:
                if self._illegal_actions_default_to_min_allowed_action:
                    player_action = min(legal_actions)
                else:
...
class ARMACGoofspielMultiAgentEnv(GoofspielMultiAgentEnv):

    def __init__(self, env_config=None):
        base_config = DEFAULT_CONFIG
        base_config["armac_variant"] = True
        env_config = with_base_config(base_config=base_config, extra_config=env_config if env_config else {})
        super(ARMACGoofspielMultiAgentEnv, self).__init__(env_config=env_config)
