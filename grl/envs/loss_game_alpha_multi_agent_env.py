...
@jit(types.UniTuple(FLOAT_DTYPE_JIT, 2)(FLOAT_DTYPE_JIT), nopython=True, fastmath=True, cache=True)
def direction_to_coord(direction):
    """takes x \in [0,1), returns 2d coords on unit circle"""
    return math.cos(2*pi*direction), math.sin(2*pi*direction)

@jit(types.UniTuple(FLOAT_DTYPE_JIT, 3)(FLOAT_DTYPE_JIT, FLOAT_DTYPE_JIT, FLOAT_DTYPE_JIT[:], FLOAT_DTYPE_JIT, FLOAT_DTYPE_JIT), nopython=True, fastmath=True, cache=True)
def loss_game(dir1, dir2, starting_coords, alpha1, alpha2):
...
class LossGameAlphaMultiAgentEnv(MultiAgentEnv):
    def __init__(self, env_config):
        ...
        self.continuous_action_space = Box(low=-1.0, high=1.0, shape=(2,), dtype=FLOAT_DTYPE_NP)
        ...
        if len(self.discrete_actions_for_players) == 2 or env_config.get("show_discrete_action_space"):
            self.action_space = self.discrete_action_space
        else:
            self.action_space = self.continuous_action_space
        self.observation_space = Box(low=-500.0, high=500.0, shape=(3,), dtype=FLOAT_DTYPE_NP)
        self.player1 = 0
        self.player2 = 1
        self.num_moves = 0
        self.current_coord = np.array([0.0, 0.0], dtype=FLOAT_DTYPE_NP)
    ...
    def _discrete_to_continuous_action(self, discrete_action):
        ...
        alpha_action = (alpha_percent * (self.continuous_action_space.high[1] - self.continuous_action_space.low[1])) + self.continuous_action_space.low[1]
        direction_action = (direction_percent * (self.continuous_action_space.high[0] - self.continuous_action_space.low[0])) + self.continuous_action_space.low[0]

        continuous_action = np.asarray([direction_action, alpha_action], dtype=np.float32)
    ...
    def step(self, action_dict):
        ...
        val, x, y = loss_game(move1, move2, self.current_coord, alpha1=np.float32(alpha1), alpha2=np.float32(alpha2))
        ...
        self.current_coord = np.array([x, y], dtype=np.float32)
        r1 = val
        r2 = -val
        rew = {
            self.player1: r1,
