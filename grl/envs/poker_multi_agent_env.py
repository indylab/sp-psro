...
def _distance_to_bet_amount(x, total_num_discrete_actions_including_dummy, player_action):
    if x in [0, 1]:
        # don't match with 0 or 1 because we have custom logic to select those actions.
        return 1000.0
    return abs(x / (total_num_discrete_actions_including_dummy - 1) - player_action)
...
def parse_discrete_poker_action_from_continuous_space(continuous_action, legal_actions_list,
    ...
    # player action should be between -1.0 and 1.0

    # fold and call have large ranges in [-1.0, -0.5] and [-0.5, 0] to be easy to choose
    if 0 in legal_actions_list and -1.0 <= continuous_action <= -0.5:
        discrete_action = 0
    elif 1 in legal_actions_list and -0.5 <= continuous_action <= 0.0:
        discrete_action = 1
    else:
        player_action = np.clip(continuous_action, a_min=0.0, a_max=1.0)

        assert 0.0 - 1e-9 <= player_action <= 1.0 + 1e-9, f"action was: {player_action} before clipping: {continuous_action}"
        # place discrete actions in [0, 1] and find closest corresponding discrete action to player action
        nearest_legal_discrete_action = min(
            legal_actions_list,
            key=lambda x: _distance_to_bet_amount(x, total_num_discrete_actions_including_dummy, player_action)
        )

        discrete_action = nearest_legal_discrete_action
...
class PokerMultiAgentEnv(ValidActionsMultiAgentEnv):
    ...
    def __init__(self, env_config=None):
        ...
        if self.game_version in [KUHN_POKER, LEDUC_POKER, "leduc_poker_10_card"]:
            ...
            max_raises = str(env_config.get("universal_poker_max_raises", 2))
            if len(max_raises) == 0:
