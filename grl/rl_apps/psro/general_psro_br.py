    ...
    class P2SROPreAndPostEpisodeCallbacks(DefaultCallbacks):
        ...
        def on_episode_end(self, *, worker: RolloutWorker, base_env: BaseEnv,
            ...
            if "zero_sum_rew" in episode.last_info_for(agent_id=worker.br_player):
                ...
                episode.custom_metrics["zero_sum_reward_best_response"] = episode.last_info_for(agent_id=worker.br_player)["zero_sum_rew"]
                episode.custom_metrics["zero_sum_reward_metanash"] = episode.last_info_for(agent_id=metanash_player)["zero_sum_rew"]
                episode.custom_metrics["tie_rate"] = episode.last_info_for(agent_id=worker.br_player)["tied"]
                episode.custom_metrics["win_rate_best_response"] = episode.last_info_for(agent_id=worker.br_player)["won"]
                episode.custom_metrics["win_rate_metanash"] = episode.last_info_for(agent_id=metanash_player)["won"]
                episode.custom_metrics["episode_steps_elapsed"] = episode.last_info_for(agent_id=metanash_player)["episode_steps_elapsed"]
        ...
        for player_to_wait_on in range(2):
            ...
            while True:
                if p2sro_manager.is_policy_fixed(player=player_to_wait_on, policy_num=active_policy_num):
                    break
                if wait_count % 10 == 0 and wait_count <= 100:
