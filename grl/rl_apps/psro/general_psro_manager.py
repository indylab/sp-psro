...
from grl.utils.common import datetime_str, ensure_dir, data_dir
...
def launch_manager(scenario: PSROScenario, psro_port: int, eval_port: int,
                   block: bool = True, include_evals: bool = True, restore_payoff_table_path: str = None) -> P2SROManagerWithServer:
    ...
    log_dir = os.path.join(data_dir(), scenario.name, f"manager_{datetime_str()}")
    ...
    if restore_payoff_table_path:
        manager = P2SROManagerWithServer(
            ...
            restore_payoff_table_json_file_path=restore_payoff_table_path
        )
    else:
        manager = P2SROManagerWithServer(
        port=psro_port,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore', type=str, required=False, default=None)
    ...
    launch_manager(scenario=scenario, psro_port=psro_port, eval_port=eval_port,
                   restore_payoff_table_path=commandline_args.restore)
