...
from grl.envs.matrix_game_multi_agent_env import get_openspiel_game_from_normal_from_game_asset_id
        ...
        if infostate_key not in avg_policy_tables[player]:
            ...
            if avg_reach_probs[player] + br_reach_probs[player] > 0.0:
                for action in legal_actions:
                    ...
                    assert not np.isnan(pr), (alpha[player] * br_reach_probs[player] *
                                                  (br_policy[action] - avg_policy[action]))
                    assert isinstance(pr, float), f"pr is {pr}"

                    avg_policy_tables[player][infostate_key][action] = pr
                    pr_sum += pr
                assert np.isclose(pr_sum, 1.0), pr_sum
            else:
                for action in legal_actions:
...
def openspiel_policy_from_nonlstm_rllib_policy(openspiel_game: OpenSpielGame,
    ...
    def policy_callable(state: pyspiel.State):
        ...
        if game_version in ["kuhn_poker", "leduc_poker", "oshi_zumo", "oshi_zumo_tiny", "universal_poker", "leduc_poker_10_card", "liars_dice", "goofspiel", "battleship", "spsro_repeated_rps", "spsro_biased_repeated_rps", "dummy_repeated_rps"]:
            ...
            if game_version in ["goofspiel", "spsro_repeated_rps", "spsro_biased_repeated_rps", "dummy_repeated_rps"]:
                # after goofspiel is converted to turn-based from simultaneous, the first 4 scalars are added to encode player turn, etc.
                info_state_vector = info_state_vector[4:]

            obs = np.concatenate(
                (np.asarray(info_state_vector, dtype=np.float32), np.asarray(valid_actions_mask, dtype=np.float32)),
                axis=0)
        elif ".pkl" in game_version:
            # after game is converted to turn-based from simultaneous, the first 4 scalars are added to encode player turn, etc.
            info_state_vector = info_state_vector[4:]
        ...
        valid_action_prob_sum, action_probs, len(action_probs), len(valid_actions_mask), action_info.get('behaviour_logits'))
...
class JointPlayerPolicy(OpenSpielPolicy):
    ...
    def action_probabilities(self, state, player_id=None):
        ...
        action_probs = player_policy.action_probabilities(state=state, player_id=cur_player)
        rounded_action_probs = {}
        for action, prob in action_probs.items():
            if np.isclose(prob, 0.0, atol=1.e-6) and prob < 0.0:
                prob = 0.0
            rounded_action_probs[action] = prob
        assert np.isclose(np.sum(list(rounded_action_probs.values())), 1.0), np.sum(list(rounded_action_probs.values()))
        return rounded_action_probs
...
def psro_measure_exploitability_nonlstm(br_checkpoint_path_tuple_list: List[Tuple[str, str]],
    ...
    # assert False, "ok to remove this assertion, just making sure battleship with multiple shots doesnt calculate exploitability."
    ...
    if poker_game_version == "spsro_repeated_rps":
        rps_matrix_game = pyspiel.load_game("matrix_rps")
        rps_repeated_matrix_game = pyspiel.create_repeated_game(rps_matrix_game,
                                                                {"num_repetitions": 4, "enable_infostate": True})
        openspiel_game = pyspiel.convert_to_turn_based(rps_repeated_matrix_game)
    elif poker_game_version == "spsro_biased_repeated_rps":
        rps_matrix_game = pyspiel.load_game("biased_matrix_rps")
        rps_repeated_matrix_game = pyspiel.create_repeated_game(rps_matrix_game,
                                                                {"num_repetitions": 4, "enable_infostate": True})
        openspiel_game = pyspiel.convert_to_turn_based(rps_repeated_matrix_game)
    elif poker_game_version == "dummy_repeated_rps":
        rps_matrix_game = pyspiel.load_game("dummy_action_matrix_rps")
        rps_repeated_matrix_game = pyspiel.create_repeated_game(rps_matrix_game,
                                                                {"num_repetitions": 4, "enable_infostate": True})
        openspiel_game = pyspiel.convert_to_turn_based(rps_repeated_matrix_game)
    elif ".pkl" in poker_game_version:
        ...
        for checkpoint_path_tuple in br_checkpoint_path_tuple_list:
            ...
            for player, player_rllib_policy in enumerate(rllib_policies):
                ...
                if checkpoint_path not in _psro_tabular_policies_cache:
                    ...
                    if CACHE_PSRO_TABULAR_POLICIES and os.path.exists(checkpoint_path):
                        # dont cache paths which don't correspond to a real file on disk,
                        # such as a special delimiter path ("sp_br"), etc. Those policies might be changing over time.
                        _psro_tabular_policies_cache[checkpoint_path] = single_openspiel_policy
                else:
                    assert "sp_br_0" not in _psro_tabular_policies_cache
...
def dream_sd_measure_exploitability_nonlstm(br_checkpoint_path_tuple_list: List[Tuple[str, str]],
                                        metanash_weights: List[Tuple[float, float]],
                                        set_policy_weights_fn: Callable,
                                        rllib_policies: List[Policy],
                                        poker_game_version: str,
