...
from ray.rllib.utils.typing import PolicyID, ModelWeights, TensorType
...
class PSROBRActor:
    ...
    def __init__(self, player: int, results_dir: str, scenario_name: str, psro_manager_port: int,
        ...
        # assert allow_stochastic_best_response, "allow_stochastic_best_response is set to False, " \
        #                                        "not sure if this will work with self-play PSRO"
            ...
            def on_episode_start(self, *, worker: RolloutWorker, base_env: BaseEnv,
                ...
                if time_for_resample and opponent_policy_distribution is not None:
                    ...
                    if sampled_spec_index == len(original_player_probs[other_player]):
                        ...
                        metanash_policy.config["explore"] = True
                        try:
                            ...
                            metanash_policy.set_weights(worker._latest_self_play_br_weights)
                        ...
                        metanash_policy.config["explore"] = scenario.allow_stochastic_best_responses
                    ...
                    if result["training_iteration"] % 10 == 0 and metasolver is not None:
                        ...
                        def _load_strat_or_sp_br_weights(rllib_policy, checkpoint_path):
                            if checkpoint_path in ["sp_br_0", "sp_br_1"]:
                                ...
                                weights = trainer.latest_sp_br_weights[_player_id]
                                rllib_policy.set_weights(weights=weights)
                            else:
        ...
        self._latest_local_sp_br_weights = self.trainer.workers.local_worker().policy_map["metanash"].get_weights()
            ...
            if scenario.calc_exploitability_for_openspiel_env and self.calculate_openspiel_exploitability_at_end and self.metasolver is not None:
                ...
                def _load_strat_or_sp_br_weights(rllib_policy, checkpoint_path):
                    if checkpoint_path in ["sp_br_0", "sp_br_1"]:
                        _player_id = int(checkpoint_path[-1])
                        assert _player_id in [0, 1]
                        weights = self.trainer.latest_sp_br_weights[_player_id]
                        rllib_policy.set_weights(weights=weights)
                    else:
                        load_pure_strat(rllib_policy, checkpoint_path=checkpoint_path)
                ...
                assert self._last_metasolver_exploitability is None
    ...
    def submit_self_play_policy_and_cleanup(self):
        ...
        final_self_play_policy_metadata = create_metadata_with_new_checkpoint_for_current_best_response(
            trainer=self.trainer, player=self.other_player, save_dir=checkpoint_dir(trainer=self.trainer),
            timesteps_training_br=self.total_timesteps_training_br,
            episodes_training_br=self.total_episodes_training_br,
            active_policy_num=-1,
            average_br_reward=-1.0,
            policy_id_to_save="metanash"
        )
    ...
    def get_sp_br_weights(self):
        return self._latest_local_sp_br_weights

    def set_sp_br_weights(self, sp_br_weights_for_each_player):
        self.trainer.latest_sp_br_weights = sp_br_weights_for_each_player
    ...
    while True:
        ...
        while True:
            if psro_iter > 0:
                ...
                latest_sp_br_weights = []
                for l in reversed(learner_references):
                    latest_sp_br_weights.append(ray.get(l.get_sp_br_weights.remote()))
                for l in learner_references:
                    ray.get(l.set_sp_br_weights.remote(latest_sp_br_weights))
