...
def nfsp_kuhn_avg_policy_params_valid_actions(env: MultiAgentEnv) -> Dict[str, Any]:
    return merge_dicts(nfsp_kuhn_avg_policy_params(env), {
        "model": merge_dicts(MODEL_DEFAULTS, {
            "fcnet_activation": "relu",
            "fcnet_hiddens": [128],
            "custom_model": get_valid_action_fcn_class_for_env(env=env),
...
def nfsp_kuhn_dqn_params_valid_actions(env: MultiAgentEnv) -> Dict[str, Any]:
    return merge_dicts(nfsp_kuhn_dqn_params(env), {
        "model": merge_dicts(MODEL_DEFAULTS, {
            "fcnet_activation": "relu",
            "fcnet_hiddens": [128],
            "custom_model": get_valid_action_fcn_class_for_env(env=env),
...
def nfsp_leduc_avg_policy_params_no_valid_actions_model(env: MultiAgentEnv) -> Dict[str, Any]:
    return {
        "framework": "torch",
        "num_gpus": float(os.getenv("WORKER_GPU_NUM", 0.0)),
        "num_workers": 0,
...
def nfsp_leduc_dqn_params_no_valid_actions_model(env: MultiAgentEnv) -> Dict[str, Any]:
...
def goofspiel_nfsp_params(env: MultiAgentEnv) -> Dict[str, Any]:
    params = nfsp_leduc_avg_policy_params(env=env)
    params["metrics_smoothing_episodes"] = 5000
    params["model"]["fcnet_hiddens"] = [128, 128, 128]
    return params