...
def data_dir() -> str:
    return os.getenv("GRL_DATA_DIR", default=os.path.join(os.path.dirname(grl.__file__), "data"))


def assets_dir() -> str:
    return os.getenv("GRL_ASSETS_DIR", default=os.path.join(os.path.dirname(grl.__file__), "assets"))


def dependencies_dir() -> str:
    return os.getenv("GRL_DEPENDENCIES_DIR", default=os.path.join(os.path.dirname(os.path.dirname(grl.__file__)), "dependencies"))
...
def timestamp_from_str(date_string) -> float:
    return datetime.datetime.strptime(date_string, "%I.%M.%S%p_%b-%d-%Y").timestamp()
